...
bool ADIGPIOCtl::wait_for_low_and_set_high_profile(uint32_t pin)
{
    ...
    while (elapsed_us < timeout_us) {
        ...
        if (value == gpio_->HIGH) {
            ...
            auto total_us = std::chrono::duration_cast<std::chrono::microseconds>(t_end - t_start)
                                .count();
        ...
        while (true) {
            ...
            auto diff = std::chrono::duration_cast<std::chrono::microseconds>(now - t_poll)
                            .count();
    ...
    , mipi_lane_speed_(mipi_lane_speed)
{
    ...
    const AdcamModeConfig* init_cfg = adcam_find_mode(adsd3100_standardModes, static_cast<int>(adcam_mode));
    if (!init_cfg) {
    ...
    std::shared_ptr<hololink::csi::CsiConverter> converter)
{
    ...
    uint32_t transmitted_line_bytes = converter->transmitted_line_bytes(csi::PixelFormat::RAW_8, width_ * pixel_width_);
