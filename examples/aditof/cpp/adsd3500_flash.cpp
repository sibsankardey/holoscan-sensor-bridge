    ...
    bool force)
{
    ...
    uint32_t master_len = (uint32_t)file_data[8] | ((uint32_t)file_data[9] << 8) | ((uint32_t)file_data[10] << 16) | ((uint32_t)file_data[11] << 24);
    ...
    uint32_t slave_len = (uint32_t)file_data[ADI_DUAL_FW_SLOT_SIZE + 8] | ((uint32_t)file_data[ADI_DUAL_FW_SLOT_SIZE + 9] << 8) | ((uint32_t)file_data[ADI_DUAL_FW_SLOT_SIZE + 10] << 16) | ((uint32_t)file_data[ADI_DUAL_FW_SLOT_SIZE + 11] << 24);
    ...
    uint32_t master_expected_crc = (uint32_t)file_data[ADI_CHUNK_HEADER_SIZE + master_len] | ((uint32_t)file_data[ADI_CHUNK_HEADER_SIZE + master_len + 1] << 8) | ((uint32_t)file_data[ADI_CHUNK_HEADER_SIZE + master_len + 2] << 16) | ((uint32_t)file_data[ADI_CHUNK_HEADER_SIZE + master_len + 3] << 24);
    ...
    std::vector<uint8_t> slave_fw(file_data.begin() + ADI_DUAL_FW_SLOT_SIZE + ADI_CHUNK_HEADER_SIZE,
    ...
    uint32_t slave_expected_crc = (uint32_t)file_data[ADI_DUAL_FW_SLOT_SIZE + ADI_CHUNK_HEADER_SIZE + slave_len] | ((uint32_t)file_data[ADI_DUAL_FW_SLOT_SIZE + ADI_CHUNK_HEADER_SIZE + slave_len + 1] << 8) | ((uint32_t)file_data[ADI_DUAL_FW_SLOT_SIZE + ADI_CHUNK_HEADER_SIZE + slave_len + 2] << 16) | ((uint32_t)file_data[ADI_DUAL_FW_SLOT_SIZE + ADI_CHUNK_HEADER_SIZE + slave_len + 3] << 24);
    ...
    std::cout << "[INFO] Slave  firmware version : " << slave_ver_str
              << std::endl;
    if (master_fw[0] != slave_fw[0] || master_fw[1] != slave_fw[1] || master_fw[2] != slave_fw[2] || master_fw[3] != slave_fw[3]) {
